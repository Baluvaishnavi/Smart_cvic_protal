...
## 🧪 Automated Testing Suite (31 Tests)
...
* `tests/test_auth_and_registration.py` (6 tests): Citizen registration, duplicate email rejection, admin email spoofing protection, login authentication, single-admin credential validation, and strict `403 Forbidden` authorization guards.
* `tests/test_api.py` (7 tests): API health check, complaint submission, lifecycle status transitions (`NEW` $\rightarrow$ `ASSIGNED` $\rightarrow$ `IN_PROGRESS` $\rightarrow$ `RESOLVED`), community upvoting, analytics computation, hotspot aggregation, and custom category handling.
* `tests/test_ai.py` (5 tests): Case study prompt extraction, NLP classification accuracy, water supply categorization, and duplicate cosine similarity.
* `tests/test_prioritization.py` (6 tests): Base severity scoring, aging penalties, SLA breach escalation logic, upvote multipliers, and priority tier classification.
* `tests/test_rbac_and_approval.py` (4 tests): Role authentication, citizen complaint data isolation, admin approval/rejection workflows, and batch approval filters.
* `tests/test_ml_trainer.py` (3 tests): Text cleaning regex, dataset record loading, and model training evaluation.

**Result: 31 of 31 tests passing (100% pass rate) in ~18 seconds.**

---

## ☁️ Cloud Deployment (Railway)

The repository is pre-configured for 1-click cloud container deployment on [Railway](https://railway.app):

1. **Deployment Files Included**:
   * `Procfile`: `web: python run.py`
   * `run.py`: Configured for host `0.0.0.0` and dynamic `$PORT`.
   * `backend/app/database.py`: Automatically consumes Railway's `MONGO_URL`, `MONGODB_URL`, or `MONGO_PRIVATE_URL`.
...
2. **Steps to Deploy**:
   * Push project to GitHub:
     ...
     git init
     git add .
     git commit -m "Deploy Smart Civic Portal"
     git branch -M main
     git remote add origin https://github.com/<YOUR_USERNAME>/smart-civic-portal.git
     git push -u origin main
     ```
   * On [railway.app](https://railway.app), click **+ New Project** $\rightarrow$ **Provision MongoDB**.
   * Click **+ New** $\rightarrow$ **GitHub Repo** $\rightarrow$ Select your repository.
   * In Web Service Settings $\rightarrow$ Variables, link `MONGO_URL = ${{MongoDB.MONGO_URL}}`.
   * In Networking, click **Generate Domain** to receive your live HTTPS public portal URL.

---

## 📄 License & Attribution
Developed for municipal governance, smart city initiatives, and capstone technical demonstrations. Released under the MIT License.
